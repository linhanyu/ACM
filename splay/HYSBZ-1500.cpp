...
//    if(x){
//        Treabel(ch[x][0]);
//        printf("node: %2d : l: %2d r: %2d fa: %2d siz: %2d\n",x,ch[x][0],ch[x][1],pre[x],size[x]);
//        Treabel(ch[x][1]);
//    }
//}
///
...
//void pushDown(int r){
//    if (same[r]){
//        updateSame(ch[r][0],key[r]);
//        updateSame(ch[r][1],key[r]);
//        same[r] = 0;
//    }
//
//    if (rev[r]){
...
//    ch[y][!kind] = ch[x][kind];
//    pre[ch[x][kind]] = y;
//    //左旋:kind = 0 ,左树过继给父节点右树
//    if (pre[y]) ch[pre[y]][GETC(y)] = x;
//    //祖父节点不是根,把节点x丢给祖父节点
//    pre[x] = pre[y];
//    ch[x][kind] = y;
//    pre[y] = x;
...
// }
//
//int getKth(int k,int r){
//    pushDown(r);
//    int t = size[ch[r][0]] + 1;
//    if (t == k) return r;
//    return t > k ? getKth(ch[r][0],k) : getKth(ch[r][1],k-t) ;
//}
//
//void Insert(int pos,int tot){
//    //第pos大的数后
...
//    Splay(getKth(pos+ tot + 1,root),root);
//    Erase(Key_value);
//    pre[Key_value] = 0;
//    Key_value = 0;
//    pushUp(ch[root][1]);
