...
#define BIT 8
#define NUM 181
using namespace std;char ans[][100] = {"1",
                                       "1",
                                       "5",
                                       "42",
                                       "462",
...
//}
//
//int BigNumAdd(int BigNum1[],int BigNum2[],int size1,int size2){
//    int i = 0;
//    for ( ;i < size1 || i < size2; ++i) {
//        BigNum1[i] += BigNum2[i];
//    }
//
//    return dealG(BigNum1,i);
//}
//
//void printBigNum(int BigNum[],int size) {
//
//    printf("%d",BigNum[size-1]);
//    for (int i = size - 2; i >= 0; --i) {
//        printf("%08d",BigNum[i]);
//    }
//}
//
//int Bigdp[NUM][NUM][NUM][12];
//int size[NUM][NUM][NUM];
//int main(){
//
//
//    memset(Bigdp,0, sizeof(Bigdp));
...
//            for (int k = i; k >= 1 ; --k) {
//                if ( k<=j && i-j-k<=k){
//                    size[i][j][k] = BigNumAdd(Bigdp[i][j][k],Bigdp[i-1][j-1][k],size[i][j][k],size[i-1][j-1][k]);
//                    size[i][j][k] = BigNumAdd(Bigdp[i][j][k],Bigdp[i-1][j][k-1],size[i][j][k],size[i-1][j][k-1]);
//                    size[i][j][k] = BigNumAdd(Bigdp[i][j][k],Bigdp[i-1][j][k],size[i][j][k],size[i-1][j][k]);
//                }
//            }
//        }
//    }
...
//    for (int i = 0; i <= 60; ++i) {
//        printf("\"");
//        printBigNum(Bigdp[i*3][i][i],size[3*i][i][i]);
//        printf("\",\n");
//    }
//}